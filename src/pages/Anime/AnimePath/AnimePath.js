...
function AnimePath() {
    ...
    if (isLoading) {
        return (
            <Container css={{ display: 'flex', justifyContent: 'left', flexWrap: 'wrap', gap: 15, marginTop: 15, '@tablet': { justifyContent: 'space-between', }, '@mobile': { justifyContent: 'center' } }}>
    ...
    if (animePath === '/videos') {
        return (
            <Container>
                <Container css={{ display: 'flex', justifyContent: 'left', flexWrap: 'wrap', gap: 20, marginTop: 15, '@tablet': { justifyContent: 'space-between', }, '@mobile': { justifyContent: 'center' } }}>
                    {data.data.episodes.map((episode) => (
                        <Container key={episode.title} css={{ width: 200, '@mobile': { width: '100%' } }}>
                            <a href={episode.url} target='_blank'>
                                <Video thumb={episode.images.jpg.small_image_url} radii css={{ width: 200, height: 110, '@mobile': { width: '100%', height: '50vw' } }} shadow={'no'} />
                            </a>
                    ...
                    {data.data.promo.map((promo) => (
                        <Container key={promo.title} css={{ width: 200, '@mobile': { width: '100%' } }}>
                            <a href={promo.trailer.url} target='_blank'>
                                <Video thumb={promo.trailer.images.small_image_url} radii css={{ width: 200, height: 110, '@mobile': { width: '100%', height: '50vw' } }} shadow={'no'} />
                            </a>
        ...
        return (
            <Container css={{ display: 'flex', justifyContent: 'left', flexWrap: 'wrap', gap: 15, marginTop: 15, '@tablet': { justifyContent: 'space-between', }, '@mobile': { justifyContent: 'center' } }}>
                {data.data.map((picture) => (
                    <Thumbnail radii key={picture.jpg.large_image_url} src={picture.jpg.large_image_url} alt={picture.jpg.large_image_url} css={{ width: 200, height: 'auto', '@mobile': { width: '100%' } }} />
                ))}
                {data.data.length === 0 && (
